...
class AkamaiToken:
    def __init__(self, token_type=None, token_name='hdnts',
                 ip=None, start_time='now', end_time=0, window_seconds=None,
...
Generating token...''' % (
                ...
                ''.join([str(x)
                         for x in [self._window_seconds] if x is not None]),
                ...
                ''.join([str(x)
                         for x in [self._field_delimiter] if x is not None]),
                ''.join([str(x)
                         for x in [self._acl_delimiter] if x is not None]),
...
class Movienight(commands.Cog):
    ...
    def create_ull_fetch_target_request(self, target_id):
        request = {'header': {"Content-Type": "application/json",
                   ...
                   'url': "{}{}/stream_targets/ull/{}".format(self.wsc_host, self.wsc_version, target_id)
                   }
        return request
    ...
    def create_stream_state_request(self, target_id):
        request = {'header': {"Content-Type": "application/json",
                   ...
                   'url': "{}{}/live_streams/{}/state".format(self.wsc_host, self.wsc_version, target_id)
                   }
        return request
    ...
    def fetch_latest_ull_target_id(self):
        ...
        for target in jres["stream_targets_ull"]:
            ...
            elif datetime.strptime(latest_target['created_at'],
                                   '%Y-%m-%dT%H:%M:%S.%fZ') < datetime.strptime(target['created_at'],
                                                                                '%Y-%m-%dT%H:%M:%S.%fZ'):
                latest_target = target
        if latest_target:
        ...
        while True:
            ...
            if jres['stream_target_ull']['state'] == "started":
                if not self.ull_stream_running:
                    ...
                    self.ull_playback_key = target["playback_urls"]["ws"][1].split(
                        '/')[5]
                    print("Detected ull stream (playback key" +
                          self.ull_playback_key + ")")

                    c = self.bot.get_channel(self.alert_channel_id)
                    embed = discord.Embed(title="Movienight is online!", color=0x0600ff,
                    ...
                    embed.add_field(
                        ...
                        value='https://movie.admiralbulldog.live/ull_player-hls-forced.html?key={}'.format(
                            self.ull_playback_key), inline=True)
            ...
            if jres['live_stream']['state'] == "started":
                if not self.cdn_stream_running:
                    ...
                    c = self.bot.get_channel(self.alert_channel_id)
                    embed = discord.Embed(title="Movienight is about to start!", color=0x0600ff,
    ...
    async def movienight(self, ctx):
        if not ctx.channel.id == 588693719646076929 and not ctx.channel.id == 557634996743962625:
            return
        ...
        if self.ull_stream_running:
            ...
            embed.set_thumbnail(url=self.logo_url)
            embed.add_field(name='Web (low latency):',
                            value='https://movie.admiralbulldog.live/ull_player.html?key={}'.format(
                                self.ull_playback_key),
                            inline=False)
            embed.add_field(
    ...
    @_moviestart.command(pass_context=True,
                         help="Creates an ultra-low-latency target (lowest latency, source quality only)")
    async def ull(self, ctx):
        if self.bd_id == ctx.author.id or self.bo_id == ctx.author.id or self.he_id == ctx.author.id:
            ...
            embed = discord.Embed(title="A new Wowza ULL target has been created! Use the following OBS settings:",
                                  color=0x008000)
    ...
    async def cdn(self, ctx):
        if self.bd_id == ctx.author.id or self.bo_id == ctx.author.id or self.he_id == ctx.author.id:
            ...
            await ctx.author.dm_channel.send("Starting transcoder, this usually takes less than a minute...")

            if self.start_cdn_target():
                embed = discord.Embed(
                    title="Wowza CDN target ready. Here are the OBS settings:", color=0x008000)
                embed.set_author(name="Movienight (CDN)")
                embed.add_field(
                    name="Server:", value="rtmp://entrypoint...", inline=False)
                embed.add_field(name="Stream Key:",
                                value="stream_key", inline=False)
                embed.add_field(name="Authentication:",
                                value="Enabled", inline=False)
                embed.add_field(name="Username:",
                                value="username", inline=False)
                embed.add_field(name="Password:",
                                value="pw", inline=False)

                await ctx.author.dm_channel.send("Target ready!", embed=embed)
            else:
                await ctx.author.dm_channel.send("Was unable to start the transcoder, please check logs")
    ...
    def create_ull_target_request(self, name):
        request = {'header': {"Content-Type": "application/json",
                   ...
                   'payload': {"stream_target_ull": {"name": name,
    ...
    def create_transcoder_start_request(self, transcoder_id):
        request = {'header': {"Content-Type": "application/json",
                   ...
                   'url': "{}{}/transcoders/{}/start".format(self.wsc_host, self.wsc_version, transcoder_id)
                   }
        return request
    ...
    def fetch_transcoder_state_request(self, transcoder_id):
        request = {'header': {"Content-Type": "application/json",
                   ...
                   'url': "{}{}/transcoders/{}/state".format(self.wsc_host, self.wsc_version, transcoder_id)
                   }
        return request
    ...
    def create_ull_target(self):
        ...
        res = requests.post(req['url'], data=json.dumps(
            req['payload']), headers=req['header'])
        jres = json.loads(res.content)
