...
class DAO:
    ...
    def append_sub_to_banlist(self, server_id, user_id):
        try:
            with self.connection.cursor() as cursor:
                ...
                sql = (
                    "UPDATE server SET ban_list_json = %s WHERE server_discord_id = %s"
                )
    ...
    def remove_sub_from_queue(self, server_id, user_id):
        try:
            with self.connection.cursor() as cursor:
                ...
                sql2 = (
                    "DELETE FROM member_queue WHERE member_fk = %s AND queue_fk = %s;"
                )
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for row in c:
                    queue.append(str(row[0]))
                    date.append(
                        datetime.datetime.strptime(str(row[1]), "%Y-%m-%d %H:%M:%S")
                    )
        finally:
            pass
        return queue, date
    ...
    async def fetch_match_details(self, match_id):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = (
                    "SELECT match_id, content_json FROM dota_matches WHERE match_id=%s;"
                )
        ...
        try:
            ...
            self.add_match_data(match_id, match_details)
    ...
    async def verify_steam(self, user_id):
        ...
        if steam_id:
            try:
                requests.post(
                    "http://localhost:3005/addfriend/", json={"steam_id": str(steam_id)}
                )
                return True
            except:
                print(
                    "Failed sending verification request to steam bot: " + str(steam_id)
                )
                return False
        else:
            return False
    ...
    def get_all_message_logs(self, server_id):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = (
                    "SELECT member_fk, stats_json FROM member_stats WHERE server_fk=%s;"
                )
                cursor.execute(sql, (server_id,))
                c = cursor.fetchall()
        finally:
