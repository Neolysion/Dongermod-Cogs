...
class Giveaway(commands.Cog):
    ...
    async def giveaway(self, ctx):
        ...
        if self.server_config["other"]["giveaway_item"] and self.server_config["other"]["giveaway_deadline"]:
            if not self.dao.get_sub_in_giveaway(str(this_author.id)):
                ...
                await ctx.send(this_author.mention + " you already joined the giveaway.")
        else:
    ...
    async def startgiveaway(self, ctx):
        ...
        await ctx.send("New giveaway was started! Use !giveawaywinner to chose a winner when the time has passed.")
    ...
    async def wipegiveaway(self, ctx):
        ...
        self.dao.update_server_config("111772771016515584", self.server_config)
        await ctx.send("The giveaway was wiped. You can start a new one with !startgiveaway")

    @commands.guild_only()
    @checks.admin_or_permissions(manage_roles=True)
    @commands.command(pass_context=True, no_pm=True, help="Chose a winner for the current giveaway")
    async def giveawaywinner(self, ctx):
        ...
        if self.server_config["other"]["giveaway_item"] and self.server_config["other"]["giveaway_deadline"]:
            ...
            await asyncio.sleep(5)
            await ctx.send("Congatulations " + win_user.mention + " you won **{}**!!!".format(
                self.server_config["other"]["giveaway_item"]))
        else:
            await ctx.send("You have to start a giveaway before you can chose a winner. Please use !startgiveaway \"<item>\" \"<deadline>\"")
    ...
    async def lockgiveaway(self, ctx):
        self.lock = True
        await ctx.send("The giveaway has been locked. Unlock again with !unlockgiveaway")
